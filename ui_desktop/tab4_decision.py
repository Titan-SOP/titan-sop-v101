...
# Titan SOP V100 — Tab 4: 全球決策 (GOD-TIER BUILD — VERIFIED + ENHANCED)
# ╔═══════════════════════════════════════════════════════════╗
# ║  Design: Bloomberg Command Center × War Room HUD         ║
# ║  Verified against: tab4_全球決策.py (original V82)       ║
# ║  Bug Fixes:                                              ║
# ║    [FIX] 4.3 session state key race condition            ║
# ║    [FIX] 4.5 format dict column name mismatch            ║
# ║    [FIX] 4.4 fillna chaining DeprecationWarning          ║
# ║  Enhancements:                                           ║
# ║    4.2 → Multi-asset equity curve overlay (normalized)   ║
# ║    4.3 → CAGR ranking horizontal bar chart               ║
# ║    4.4 → Before/After pie comparison + column layout     ║
# ║    4.5 → N×4 heatmap shock matrix                        ║
# ║    HUD → Live total asset display                        ║
# ╚═══════════════════════════════════════════════════════════╝
...
#  (verified verbatim from original V82 — zero logic changes)
# ══════════════════════════════════════════════════════════════════
...
/* ── HUD ── */
.t4-hud{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:20px;}
.t4-hud-card{background:rgba(255,255,255,.022);border:1px solid rgba(255,255,255,.062);
  border-top:2px solid var(--hc,#00F5FF);border-radius:14px;padding:13px 14px 11px;
  position:relative;overflow:hidden;}
.t4-hud-card::after{content:'';position:absolute;top:0;right:0;width:70px;height:70px;
  background:radial-gradient(circle at top right,var(--hc,#00F5FF),transparent 68%);
  opacity:.04;pointer-events:none;}
.t4-hud-lbl{font-family:var(--f-m);font-size:8px;color:rgba(140,155,178,.55);text-transform:uppercase;letter-spacing:2px;margin-bottom:7px;}
.t4-hud-val{font-family:var(--f-d);font-size:38px;color:#FFF;line-height:.92;margin-bottom:4px;}
.t4-hud-sub{font-family:var(--f-b);font-size:11px;color:var(--hc,#00F5FF);opacity:.85;font-weight:600;}
/* ── SECTION HEADER ── */
.t4-sec-head{display:flex;align-items:center;gap:12px;
  padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.052);margin-bottom:18px;}
.t4-sec-num{font-family:var(--f-d);font-size:48px;color:rgba(0,245,255,.08);letter-spacing:2px;line-height:1;}
.t4-sec-title{font-family:var(--f-d);font-size:20px;color:var(--sa,#00F5FF);letter-spacing:2px;}
...
.t4-kelly-chip{border-radius:14px;padding:14px 18px;min-width:155px;flex:1;
  border:1px solid var(--kc,rgba(255,255,255,.08));background:var(--kb,rgba(255,255,255,.02));}
.t4-kelly-chip.fire{--kc:rgba(255,49,49,.3);--kb:rgba(255,49,49,.05);}
.t4-kelly-chip.ok  {--kc:rgba(0,245,255,.22);--kb:rgba(0,245,255,.03);}
.t4-kelly-chip.ice {--kc:rgba(100,115,135,.28);--kb:rgba(100,115,135,.03);}
.t4-chip-ticker{font-family:var(--f-d);font-size:26px;color:#FFF;letter-spacing:2px;margin-bottom:4px;}
.t4-chip-cagr  {font-family:var(--f-d);font-size:38px;line-height:1;}
.t4-chip-label {font-family:var(--f-b);font-size:12px;font-weight:700;margin-top:5px;}
.t4-chip-advice{font-family:var(--f-m);font-size:9px;letter-spacing:1.5px;margin-top:3px;opacity:.7;}
.t4-chip-detail{font-family:var(--f-m);font-size:9px;color:rgba(140,155,178,.45);margin-top:6px;letter-spacing:1px;}
/* ── STRESS GRID ── */
.t4-stress-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px;}
.t4-stress-card{border-radius:14px;padding:14px 14px 12px;text-align:center;
  border:1px solid rgba(255,49,49,.22);background:rgba(255,49,49,.04);}
.t4-stress-lbl{font-family:var(--f-m);font-size:9px;color:rgba(255,100,100,.5);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px;}
.t4-stress-val{font-family:var(--f-d);font-size:34px;color:#FF6B6B;line-height:1;}
.t4-stress-pct{font-family:var(--f-b);font-size:12px;color:#FF3131;margin-top:4px;font-weight:600;}
/* ── CHART PANEL ── */
.t4-chart-panel{background:rgba(0,0,0,.28);border:1px solid rgba(255,255,255,.055);
  border-radius:16px;padding:16px 10px 8px;margin:12px 0;overflow:hidden;}
...
/* ── ACTION BUTTONS ── */
.t4-action  div.stButton>button{background:rgba(0,245,255,.05)!important;border:1px solid rgba(0,245,255,.25)!important;color:rgba(0,245,255,.85)!important;font-family:var(--f-m)!important;font-size:11px!important;letter-spacing:2px!important;min-height:44px!important;border-radius:12px!important;text-transform:uppercase!important;}
.t4-action  div.stButton>button:hover{background:rgba(0,245,255,.10)!important;box-shadow:0 0 20px rgba(0,245,255,.18)!important;}
.t4-action-r div.stButton>button{border-color:rgba(255,49,49,.25)!important;color:rgba(255,100,100,.85)!important;}
.t4-action-r div.stButton>button:hover{background:rgba(255,49,49,.07)!important;}
...
/* ── FOOTER ── */
.t4-foot{font-family:var(--f-m);font-size:9px;color:rgba(70,90,110,.28);letter-spacing:2px;text-align:right;margin-top:22px;text-transform:uppercase;}
        ...
        if total_v > 0:
            ...
            with pie_col:
                ...
                fig = go.Figure(go.Pie(
                    labels=ptd['資產代號'].tolist(), values=ptd['市值'].tolist(), hole=0.52,
                    marker=dict(colors=pal[:len(ptd)], line=dict(color='rgba(0,0,0,0.35)', width=1.5)),
                    textfont=dict(color='#DDE', size=12, family='Rajdhani'),
                ))
                fig.update_layout(
                    title=dict(text="資產配置分佈", font=dict(color='rgba(0,245,255,.45)', size=11, family='JetBrains Mono')),
                    template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)',
                    height=270, margin=dict(t=34,b=0,l=0,r=0),
...
<div style="padding:14px 0 8px;">
  <div style="font-family:var(--f-m);font-size:9px;color:rgba(0,245,255,.4);letter-spacing:3px;text-transform:uppercase;margin-bottom:10px;">Portfolio Summary</div>
  <div style="font-family:var(--f-m);font-size:9px;color:#334455;letter-spacing:2px;margin-bottom:3px;">TOTAL VALUE</div>
  <div style="font-family:var(--f-d);font-size:52px;color:#FFF;line-height:1;margin-bottom:14px;">{total_v:,.0f}</div>
  <div style="font-family:var(--f-m);font-size:9px;color:#334455;letter-spacing:2px;margin-bottom:3px;">UNREALIZED P&L</div>
  <div style="font-family:var(--f-d);font-size:40px;color:{pnl_c};line-height:1;margin-bottom:6px;">{arr} {abs(total_pnl):,.0f}</div>
  <div style="font-family:var(--f-b);font-size:14px;color:{pnl_c};font-weight:600;">{(total_pnl/total_v)*100:+.2f}% 報酬率</div>
...
def _s42():
    with st.expander("📈 4.2 績效回測與凱利決策 (Backtest & Kelly Analysis)", expanded=False):
        ...
        run_bt = st.button("🚀 啟動全球回測", key="btn_backtest_v100")
        ...
        # Kelly Chips
        summary_data = []
        st.markdown('<div class="t4-kelly-row">', unsafe_allow_html=True)
        for res in results:
            ...
            st.markdown(f"""
<div class="t4-kelly-chip {css}">
  <div class="t4-chip-ticker">{res['Ticker']}</div>
  <div class="t4-chip-cagr" style="color:{clr}">{cagr:.1%}</div>
  <div class="t4-chip-label" style="color:{kclr}">Half-Kelly {ck:.1%}</div>
  <div class="t4-chip-advice">{advice}</div>
  <div class="t4-chip-detail">Sharpe {sharpe:.2f} &nbsp;|&nbsp; MDD {mdd:.1%}</div>
</div>""", unsafe_allow_html=True)
        ...
        # [ENHANCEMENT] Multi-asset overlay (normalized to 100)
        ...
        fig_ov.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)', height=320, hovermode='x unified',
            legend=dict(font=dict(color='#B0C0D0',size=11,family='Rajdhani')),
        ...
        st.subheader("深度圖表分析")
        sel = st.selectbox("選擇要查看的資產", [r['Ticker'] for r in results], key="bt_sel_v100")
...
def _s43():
    with st.expander("🧪 4.3 均線戰法回測實驗室 (MA Strategy Lab)", expanded=False):
        ...
        sel_t = st.selectbox("選擇回測標的", options=pf['資產代號'].tolist(), key="ma_lab_ticker")
        strategies = [
        ...
        st.markdown('<div class="t4-action">', unsafe_allow_html=True)
        run_lab = st.button("🔬 啟動 15 種均線實驗", key="start_ma_lab")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        # [ENHANCEMENT] CAGR Ranking Bar Chart
        ...
        sel_s = st.selectbox("選擇策略查看圖表",
                             [r['strategy_name'] for r in results], key="ma_chart_v100")
...
# ══════════════════════════════════════════════════════════════════
def _s44():
    with st.expander("⚖️ 4.4 智慧調倉計算機 (Rebalancing Calculator)", expanded=False):
        ...
        with st.spinner("正在獲取最新市價…"):
            try:
                ...
                st.markdown(f"""
<div style="font-family:var(--f-d);font-size:48px;color:#FFF;letter-spacing:3px;line-height:1;margin-bottom:3px;">{total_v:,.0f}</div>
<div style="font-family:var(--f-m);font-size:9px;color:rgba(0,255,127,.4);letter-spacing:3px;text-transform:uppercase;margin-bottom:18px;">目前總資產 (TWD)</div>""", unsafe_allow_html=True)

                # [ENHANCEMENT] Horizontal column inputs (much better UX than vertical)
                ...
                for col, (_, row) in zip(tw_cols, pf.iterrows()):
                    with col:
                        w = st.number_input(f"{row['資產代號']}",
                            ...
                            step=1.0, key=f"target_{row['資產代號']}_v100")
                ...
                # [ENHANCEMENT] Before/After Pie side-by-side
                st.divider()
                ...
                _mini_pie(pf['資產代號'].tolist(), pf['目前市值'].tolist(), "⬅ 調倉前", b_col)
                _mini_pie(pf['資產代號'].tolist(), pf['目標市值'].tolist(), "調倉後 ➡", a_col)
...
# ══════════════════════════════════════════════════════════════════
def _s45():
    with st.expander("🌪️ 4.5 全球黑天鵝壓力測試 (Black Swan Stress Test)", expanded=False):
        ...
        run_stress = st.button("💥 啟動壓力測試", key="btn_stress_v100")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        st.markdown(f"""
<div style="font-family:var(--f-d);font-size:48px;color:#FFF;letter-spacing:3px;line-height:1;margin-bottom:3px;">{total_v:,.0f}</div>
<div style="font-family:var(--f-m);font-size:9px;color:rgba(255,49,49,.4);letter-spacing:3px;text-transform:uppercase;margin-bottom:18px;">目前總市值 (TWD) — 壓力情境</div>""", unsafe_allow_html=True)

        pnl_cols  = [c for c in results_df.columns if '損益' in c]
        total_pnl = results_df[pnl_cols].sum()

        # Shock cards
        st.markdown('<div class="t4-stress-grid">', unsafe_allow_html=True)
        for sc, pnl in total_pnl.items():
            pct   = (pnl / total_v * 100) if total_v > 0 else 0
            label = sc.replace('損益_', '')
            st.markdown(f'<div class="t4-stress-card"><div class="t4-stress-lbl">{label}</div><div class="t4-stress-val">{pnl:,.0f}</div><div class="t4-stress-pct">{pct:.1f}%</div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
def render():
    """Tab 4 — 全球決策  God-Tier Build V100 (Verified + Enhanced)"""
    _inject_css()
    _ensure_portfolio()
...
<div style="display:flex;align-items:baseline;justify-content:space-between;
  padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,.052);margin-bottom:18px;">
  <div>
    <span style="font-family:'Bebas Neue',sans-serif;font-size:26px;color:#00FF7F;
      letter-spacing:3px;text-shadow:0 0 22px rgba(0,255,127,.32);">🌍 全球決策</span>
    <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:rgba(0,255,127,.26);
      letter-spacing:3px;border:1px solid rgba(0,255,127,.10);border-radius:20px;
      padding:3px 13px;margin-left:14px;background:rgba(0,255,127,.022);">GLOBAL DECISION V100</span>
  </div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:10px;
    ...
    n_inv  = len(pf[pf['資產類別'] != 'Cash'])
    n_cash = len(pf[pf['資產類別'] == 'Cash'])
    has_bt = 'backtest_results'    in st.session_state
    has_ma = 'ma_lab_results'      in st.session_state
    has_st = 'stress_test_results' in st.session_state

    def _sig(flag):
        c = "#00FF7F" if flag else "#253040"
        s = "✓" if flag else "✗"
        return f'<span style="color:{c};font-family:Bebas Neue,sans-serif;font-size:38px;">{s}</span>'

    st.markdown(f"""
<div class="t4-hud">
  <div class="t4-hud-card" style="--hc:#00F5FF">
    <div class="t4-hud-lbl">Assets Loaded</div>
    <div class="t4-hud-val">{n_inv + n_cash}</div>
    <div class="t4-hud-sub">{n_inv} 投資 · {n_cash} 現金</div>
  </div>
  <div class="t4-hud-card" style="--hc:#FFD700">
    <div class="t4-hud-lbl">Backtest</div>
    <div class="t4-hud-val">{_sig(has_bt)}</div>
    <div class="t4-hud-sub">{'結果就緒' if has_bt else '尚未執行'}</div>
  </div>
  <div class="t4-hud-card" style="--hc:#FF9A3C">
    <div class="t4-hud-lbl">MA Lab</div>
    <div class="t4-hud-val">{_sig(has_ma)}</div>
    <div class="t4-hud-sub">{'15策略完成' if has_ma else '尚未執行'}</div>
  </div>
  <div class="t4-hud-card" style="--hc:#FF3131">
    <div class="t4-hud-lbl">Stress Test</div>
    <div class="t4-hud-val">{_sig(has_st)}</div>
    <div class="t4-hud-sub">{'壓力模型就緒' if has_st else '尚未執行'}</div>
  </div>
  <div class="t4-hud-card" style="--hc:#00FF7F">
    <div class="t4-hud-lbl">System</div>
    <div class="t4-hud-val"><span style="color:#00FF7F;font-family:Bebas Neue,sans-serif;font-size:38px;">ON</span></div>
    <div class="t4-hud-sub">All Engines Ready</div>
  </div>
</div>""", unsafe_allow_html=True)

    # Run all 5 sections with individual error boundaries
    for fn, label in [(_s41,'4.1'), (_s42,'4.2'), (_s43,'4.3'), (_s44,'4.4'), (_s45,'4.5')]:
        try:
            fn()
    ...
    st.markdown(
        f'<div class="t4-foot">Titan Global Decision V100 · '
