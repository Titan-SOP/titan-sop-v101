...
def _show_tactical_guide():
    ...
    **核心功能**：
    - **🎬 2.0 籌碼序幕 (CHIPS PROLOGUE)**：市場戰場地圖，互動式散點圖，X軸為轉換溢價率（成本），Y軸為收盤價（價值），顏色顯示已轉換比例熱度。快速鎖定狙擊區與避雷區。
    - **📡 2.1 自動獵殺 (AUTO SCAN)**：全市場雙軌普查 (.TW/.TWO)，自動篩選 SOP 黃金標準標的 (價格<120 + 多頭排列 + 轉換率<30%)。含 6 大策略面板：全市場 / SOP菁英 / 新券蜜月 / 滿年沈澱 / 賣回保衛 / 產業風口。
    - **📈 2.2 核心檢核 (SNIPER SCOPE)**：輸入 CB 代號即時拉取 K 線 + 87MA/284MA，搭配四大天條檢核卡 (價格/趨勢/轉換率/評分)。
    - **⚠️ 2.3 風險雷達 / 💰 2.4 資金配置**：負面表列警示 (籌碼鬆動/高溢價/流動性陷阱) + Top 5 等權重 20% 資金配置試算。
    - **🛠️ 2.5 戰略兵工廠 (NEW!)**：情報獵殺分析 + CBAS 槓桿試算儀 + 戰略行事曆（整合自 Tab 5）。

    **操作方式**：點擊上方選單切換模組 (2.0 籌碼序幕 → 2.1 自動獵殺 → 2.2 核心檢核 → 2.3 風險雷達 → 2.4 資金配置 → 2.5 兵工廠)。

    **狀態監控**：搭配 Tab 1 宏觀燈號使用，紅燈期間暫停所有新倉操作。

    ---
    *建議：先至 2.1 自動獵殺掃描標的 → 2.2 核心檢核逐一驗證 → 2.4 資金配置確認部位*
    """)
